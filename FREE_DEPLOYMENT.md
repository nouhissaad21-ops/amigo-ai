...
## 2. انشر من GitHub إلى Render

1. افتح [Render Dashboard](https://dashboard.render.com/) وسجّل بحساب GitHub.
2. اسمح لتطبيق Render بالوصول إلى المستودع الخاص `amigo-ai`.
3. اختر **New → Blueprint** ثم اختر مستودع `amigo-ai`.
4. سيقرأ Render ملف `render.yaml` تلقائياً ويطلب قيمتين فقط:
...
5. اضغط **Apply** وانتظر اكتمال البناء. أول بناء قد يأخذ عدة دقائق.
6. افتح خدمة `amigo-ai` ثم اضغط الرابط الذي ينتهي بـ`.onrender.com`.
7. اختر **افتح متجرك** وأنشئ أول حساب.

Render يولّد تلقائياً كلمات سر قاعدة الدور المحدود ومفاتيح JWT والتشفير وOAuth وWebhook verify token. لا تغيّر `CREDENTIAL_ENCRYPTION_KEY` بعد تخزين رموز القنوات، لأن تغييره يجعل فكها غير ممكن من دون عملية تدوير مخصّصة.

## 3. فعّل Meta عندما يصبح الموقع جاهزاً
...
إذا فشل النشر افتح **Logs** في خدمة `amigo-ai`. أكثر الأخطاء شيوعاً هي رابط Neon ناقص، مفتاح Groq ملغى، أو عدم منح Render حق الوصول إلى المستودع الخاص.
